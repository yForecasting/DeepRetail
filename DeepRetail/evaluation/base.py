...
class Evaluator(object):
   """
    ...
    Args:
        ...
        format (str, optional): The format of the input DataFrame. Can be either 'pivoted' or 'transaction'. Defaults to 'pivoted'.
    ...
    def evaluate(self, metrics, group_scores_by=["unique_id", "Model", "fh", "cv"]):
        ...
        Args:
            ...
            group_scores_by (list): A list of columns to group the predictions by. The default value groups by unique_id,
                                    Model, fh, and cv.

        Returns:
            pd.DataFrame: A DataFrame containing the scores for each group, with each row representing a single group and
                        each column representing a metric. The DataFrame also contains the columns used for grouping
                        the predictions.
